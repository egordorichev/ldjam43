...
function Entity:flash(n, r, g, b)
	...
	self.flashColor[1] = r or 1
	self.flashColor[2] = g or 1
	self.flashColor[3] = b or 1
end
...
function Entity:playSound(filename, gain, always)
	...
	if self.maxVelocity and math.abs(self.velocity.x) > self.maxVelocity.x then
		self.velocity.x = self.maxVelocity.x * lume.sign(self.velocity.x)
	end

	if self.maxVelocity and math.abs(self.velocity.y) > self.maxVelocity.y then
